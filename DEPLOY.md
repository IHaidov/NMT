...
### Збереження бази питань (редактор завдань)

Щоб **редактор завдань** (кабінет вчителя → Редактор завдань) міг зберігати зміни на сервері, додайте ще одну змінну середовища:

- **Key**: `QUESTIONS_FILE`
- **Value**: `/data/questions.json`

При першому запуску сервер скопіює поточний `db.json` у `/data/questions.json`, якщо файлу ще немає. Далі тест і база питань читають завдання з `/data/questions.json`. Збереження з редактора (кнопка «Зберегти базу на сервер») доступне лише **адміністратору** (вчитель з правами адміна).
